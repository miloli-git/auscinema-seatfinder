...
- v1 lenses (4): **best seats left / best screen / soonest / best turnout**. "Least crowded" deferred.
- **Turnout = the largest adjacent (sit-together) block bookable for that day's headcount** — it is
  coupled to seats-together, not just "can get a ticket".
- Context capture: solo gets **assumed defaults, skip straight to results**; the organiser sets a
  **per-day headcount** (a slider per candidate day) because their counts vary by day and can't be
  defaulted.
...
| 1. Light context | Solo: confirms/edits near-me, when-ish, party, seat pref (defaulted). **Organiser: sets per-day headcount** (slider per candidate day — 6 Tue, 7 Wed) | "Here's roughly my night / who can come when" | Cooperative if it's quick | Status quo: re-enter on every chain site; no way to hold day-varying headcount at all |
...
| 3. Pick the tradeoff that matters | Chooses a lens: best seats / best screen / soonest / **best turnout** | "Tonight I care most about good seats" / "getting the most of us in together" | In control | No existing tool lets you sort by *why*, or by who can come |
...
- **Group can't all sit together** on a given day — say "fits 6 of your 7 here", offer the day/session
  where all 7 do.
- Film only showing far away / at bad times — surface the real constraint, don't fake options.
- Best turnout day and best seats day disagree — present as an explicit tradeoff, do not silently pick.
...
2. My context controls: where, when-ish, seat preference; party size for solo, **per-day headcount
   (sliders) for the organiser** (light, editable, persistent).
3. The recommended ways to see it — a small ranked-by-chosen-tradeoff set, each with a one-line "why".
4. The tradeoff lens switcher (best seats / best screen / soonest / **best turnout**).
...
- **Context capture** — lightweight, inline, not a multi-step wizard.
  ...
  - *Organiser:* a small panel of candidate days, each with a **headcount slider** (how many keen that
    day). This is the one input the organiser can't skip, because the counts vary by day.
...
- **Option detail** — seat-quality map/representation, availability confidence, format/class, price
  signal, "best zones still open", **group-fit ("all 7 together" / "fits 6 of 7")**, handoff button.
...
- **"Best turnout"** = for a session's day, the largest **adjacent** block of available seats vs that
  day's keen count → `min(keen_that_day, largest_adjacent_block)`. Needs true geometry to be reliable
  (Event/Reading/Village have it; Hoyts is array-order so adjacency is approximate — flag lower confidence).
...
## Decisions resolved
- **Lenses:** four for v1 — best seats left / best screen / soonest / best turnout. "Least crowded" deferred.
- **Turnout:** largest adjacent (sit-together) block for the day's headcount.
- **Context:** solo defaults + skip-to-results; organiser sets per-day headcount sliders.
